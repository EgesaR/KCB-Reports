...
export default function SettingsPage() {
  ...
  const toggleSetting = useCallback((key: keyof typeof formValues) => {
    setFormValues((prev) => ({ ...prev, [key]: !prev[key] }));
  }, []);

  const handleInputChange = useCallback(
    (key: keyof typeof formValues, value: string) => {
  ...
  const renderSettingInput = useCallback(
        ...
        case "toggle":
          return (
            <ToggleInput
              id={setting.id}
              checked={
                formValues[setting.id as keyof typeof formValues] as boolean
              }
              onChange={() =>
                toggleSetting(setting.id as keyof typeof formValues)
              }
            />
          );
        case "text":
        case "password":
          return (
            <TextInput
              id={setting.id}
              value={
                formValues[setting.id as keyof typeof formValues] as string
              }
              onChange={(val) =>
                handleInputChange(setting.id as keyof typeof formValues, val)
              }
              type={setting.type}
    ...
    [formValues, handleInputChange, toggleSetting]
  );
        ...
        <Tab.Group>
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[280px_1fr]">
            <motion.div
              ...
              className="lg:sticky lg:top-8 lg:h-[calc(100vh-4rem)] lg:overflow-y-auto"
            >
